...
│   ├── 서비스 소개 섹션
│   ├── [테스트 시작하기] 버튼 (Primary)
│   └── [40종류 위스키 모음(기제)] 버튼 (Secondary, 다른 색상)
│
├── Catalog (별도 라우트 없음, Home과 같은 `/` 위에서 상태 전환)
...
| 버튼 | `[내 위스키 찾으러 가기 →]` (Primary CTA, 앰버색) / `[40종류 위스키 모음(기제)]` (Secondary, 하늘색 계열로 시각적 구분) |
| 입력값 | 없음 |
...
### 4-1-1. Catalog (별도 라우트 없음)

**목적**: 테스트 없이도 40종 위스키 전체를 카테고리별로 훑어볼 수 있게 한다.

| 구분 | 내용 |
| --- | --- |
| UI 구성요소 | 상단 `[홈으로]` 버튼 + 타이틀, 카테고리 필터 칩(가로 스크롤), 선택된 카테고리의 위스키 수, 위스키 카드 그리드(국문/영문명, 타입 배지, 한 줄 추천, 국가·도수·난이도·가격) |
| 버튼 | 카테고리 필터 칩(전체/싱글몰트/블렌디드/블렌디드몰트/버번/피트·스모키/쉐리/프루티/하이볼 추천), `[홈으로]` |
| 입력값 | 카테고리 선택(단일 선택) |
| 동작 | 1. 기본값은 "전체"(40종) 표시<br>2. 카테고리 칩 클릭 시 `lib/whiskyCategories.ts`에 정의된 조건(`type` 필드 또는 `flavor` 축 임계값, `highballFriendly`)으로 즉시 필터링<br>3. `[홈으로]` 클릭 시 `stage`를 `home`으로 전환 |
...
- 테스트 없이 40종 위스키를 카테고리별로 탐색하는 카탈로그 화면 (구현 완료)
